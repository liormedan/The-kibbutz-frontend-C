...
const info=await p.evaluate(()=>({lang:document.documentElement.lang,dir:document.documentElement.dir}));
console.log("html",info,"errs",errs.length,errs.slice(0,2));
await p.screenshot({path:join(__dirname,"en-home.png")});
await p.goto(BASE+"/settings",{waitUntil:"load"});await p.waitForTimeout(1200);
await p.screenshot({path:join(__dirname,"en-settings.png")});
