...
export default function NdaInboxPage() {
  ...
  if (viewingNda) {
    return (
      <main className="min-h-screen bg-background p-4 md:p-6" dir={dir}>
    ...
    <main className="min-h-screen bg-background p-6" dir={dir}>
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">{t("ndaInboxTitle")}</h1>
            <p className="text-sm text-muted-foreground">{t("ndaInboxSubtitle")}</p>
          </div>
        </div>
        ...
        <div className="mb-5 flex gap-2">
          {(["pending", "history"] as const).map(tab => (
            <button type="button" key={tab} onClick={() => setActiveTab(tab)} className={`rounded-xl px-4 py-2 text-sm font-medium ${activeTab === tab ? "bg-primary text-white" : "border border-border bg-background text-muted-foreground"}`}>
              {tab === "pending" ? t("ndaTabPending") : t("ndaTabHistory")}
            </button>
          ))}
        </div>
        ...
        {filtered.length === 0 ? (
          <EmptyState
            ...
            title={activeTab === "pending" ? t("ndaEmptyPending") : t("ndaEmptyHistory")}
            description={t("ndaEmptyDesc")}
          />
        ) : filtered.map(nda => (
          <div key={nda.id} className="glass-card mb-3 rounded-2xl p-5">
            ...
            <div className="mt-3 flex gap-2">
              ...
              {nda.status === "pending" && (
                <>
                  <button type="button" onClick={() => updateStatus(nda.id, "signed")} className="rounded-lg bg-secondary px-3 py-1.5 text-xs text-white">{t("ndaApprove")}</button>
                  <button type="button" onClick={() => updateStatus(nda.id, "rejected")} className="rounded-lg bg-red-50 px-3 py-1.5 text-xs text-red-500">{t("ndaReject")}</button>
                </>
              )}
            </div>
          </div>
...
function StatusBadge({ status }: { status: NdaStatus }) {
  ...
  const labels: Record<NdaStatus, string> = {
    pending: t("ndaStatusPending"),
    signed: t("ndaStatusSigned"),
    rejected: t("ndaStatusRejected"),
  };
