  ...
  return (
    <div className="min-h-screen bg-background p-6 animate-pulse" dir="rtl">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="w-40 h-7 rounded-full bg-slate-700/50 mb-1" />
        <div className="w-56 h-4 rounded-full bg-slate-700/50 mb-6" />
        <div className="flex gap-2 mb-5">
          {[0, 1, 2].map((i) => (
            <div key={i} className="w-20 h-8 rounded-xl bg-slate-700/50" />
          ))}
        </div>
          ...
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="glass-panel rounded-xl p-4 border border-border flex items-center justify-between gap-4">
              ...
              <div className="flex-1 space-y-1">
                <div className="w-36 h-4 rounded-full bg-slate-700/50" />
                <div className="w-24 h-3 rounded-full bg-slate-700/50" />
              </div>
              <div className="flex gap-2 flex-shrink-0">
                <div className="w-16 h-8 rounded-xl bg-slate-700/50" />
                <div className="w-16 h-8 rounded-xl bg-slate-700/50" />
              </div>
            </div>
          ))}
